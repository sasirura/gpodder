...
export async function buildSettingsUrl(scope: Scope, params?: QueryParams): Promise<string> {
    const username = getUsername()
    const config = await getClientConfig();

    const url = new URL(`${config.mygpo.baseurl}api/2/settings/${username}/${scope}.json`);
...
export async function getSettings(scope: Scope, params?: QueryParams): Promise<Record<string, any>> {
