...
import type { SubscriptionDeltaResponse, SubscriptionList } from "../types/subscription.js";
...
export async function uploadSubscriptions(subscriptions: string[]): Promise<void> {
    const username = getUsername()
    const deviceId = getDeviceId()
    const config = await getClientConfig();
...
export async function updateSubscriptionsDelta(changes: { add?: string[]; remove?: string[] }): Promise<SubscriptionDeltaResponse> {
    const username = getUsername()
    const deviceId = getDeviceId()
    const config = await getClientConfig();
...
export async function getSubscriptionChanges(since: number): Promise<SubscriptionDeltaResponse> {
    const username = getUsername()
    const deviceId = getDeviceId()
    const config = await getClientConfig();

    const url = new URL(`${config.mygpo.baseurl}subscriptions/${username}/${deviceId}.json`);
